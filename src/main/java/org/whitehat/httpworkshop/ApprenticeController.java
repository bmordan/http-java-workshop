...
public class ApprenticeController {
    ...
    String patchChallenge(Guest guests, @PathVariable String hash, HttpServletResponse res) {
        if (apprenticesHashMap.containsKey(hash)) {
            ...
            res.setStatus(404);
            return "The problem is I can find you in my HashMap :(. Go back to the POST step and try again";
        }        
    }
    ...
    String queryParamsChallenge(@PathVariable String hash, @RequestParam String starter, @RequestParam String main, @RequestParam String dessert, HttpServletResponse res) {
        if (apprenticesHashMap.containsKey(hash)) {
            Apprentice apprentice = apprenticesHashMap.get(hash);
            String reply = String.format("What a party %s! I can imagine %s enjoying %s followed by %s and finally %s will be a lot of fun!\n\nAre you OK to do the washing up? Can you complete this challenge with a DELETE request to '/apprentices/{Your-Id}'.", apprentice.getName(), String.join(", ", apprentice.getGuests()), starter, main, dessert);
            return reply;
        } else {
            res.setStatus(404);
    ...
    @DeleteMapping("/apprentices/{hash}")
    String finalDeleteStep(@PathVariable String hash, HttpServletResponse res) {
        if (apprenticesHashMap.containsKey(hash)) {
            Apprentice apprentice = apprenticesHashMap.get(hash);
            String reply = String.format("Well done %s. You have successfully completed a series of HTTP requests, using different methods, request parameters, different body encodings and encoded query parameters.\n\nAll of this forms a very important basis for really understanding REST and user authentication systems. https://applied.whitehat.org.uk", apprentice.getName());
            apprenticesHashMap.remove(hash);
            res.setStatus(204);
            return reply;
        } else {
            res.setStatus(404);
            return "🧑🏾‍💻 Is that the right Your-Id? Try again, or start again.";             
        }
    }
}
